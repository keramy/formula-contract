...
import { getProjectTeamMembers, updateReportShares, type Report, type ReportLine } from "./reports/actions";

// Types for local state management
interface LocalSection {
  id: string; // Can be existing ID or local temp ID
  originalId?: string; // Original database ID if editing existing section
  title: string;
  description: string;
  photos: string[];
  isNew?: boolean; // Flag for newly added sections
}
...
function SortableSection({ section, onEdit, onDelete, disabled }: SortableSectionProps) {
  ...
  return (
    <Card
      ref={setNodeRef}
      style={style}
      className={`p-3 ${isDragging ? "shadow-lg ring-2 ring-teal-500" : "hover:bg-muted/30"}`}
    >
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const originalFile of Array.from(files)) {
      ...
      try {
        ...
        const fileName = `${projectId}/${report.id}/${Date.now()}-${Math.random().toString(36).slice(2)}.${fileExt}`;
